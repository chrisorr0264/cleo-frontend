...
class build:
    ...
    def __init__(self, *directory, recursive=True, in_folder=False, limit_extensions=True, px_size=50, show_progress=True, processes=None, **kwargs):
        ...
        logging.info("Initializing difPy build with directories: %s", directory)
    ...
    def _main(self):
        ...
        logging.info("Valid files found: %d, Skipped files: %d", len(valid_files), len(skipped_files))
        ...
        logging.info("Finished building image dictionaries.")
    ...
    def _build_image_dictionaries(self, valid_files):
        ...
        count = 0
        logging.info("Starting tensor generation for valid files.")
            ...
            with Pool(processes=self.__processes) as pool:
                ...
                for output in pool.starmap(self._generate_tensor, file_nums):
                    if isinstance(output, dict):
                        ...
                        tensor_dictionary.update({img_id : tensor})
                        count += 1
                        # Log successful tensor generation
                        logging.debug(f"Generated tensor for image: {valid_files[filename]} with shape {shape}")
                        logging.debug(f"Tensor size: {tensor.size} for image: {valid_files[filename]}")
        
        # Log completion of tensor generation
        logging.info("Completed tensor generation for %d images", count)
    ...
    def _generate_tensor(self, num, file):
        ...
        try:
            ...
            # Log the error if tensor generation fails
            logging.error(f"Failed to generate tensor for image {file}: {str(e)}")
...
class search:
    ...
    def __init__(self, difpy_obj, similarity='duplicates', rotate=True, lazy=True, show_progress=True, processes=None, chunksize=None, **kwargs):
        # Logging the initialization
        logging.info("Initializing difPy search with similarity: %s", similarity)
    ...
    def _search_union(self):
        ...
        if len(self.__difpy_obj._tensor_dictionary.keys()) <= 5000:
            ...
            # search algorithm for larger datasets, > 5k images
    ...
    def _find_matches(self, ids):
        ...
        try:
            ...
            logging.error(f"KeyError: {str(e)} - Image ID {e.args[0]} not found in tensor_dictionary.")
            return False
    ...
    def _find_matches_batch(self, ids):
        ...
        try:
            ...
            logging.error(f"KeyError: {str(e)} - Image ID {id_A} not found in tensor_dictionary.")
            return []
        ...
        try:
            ...
            logging.error(f"KeyError: {str(e)} - One of the IDs in {ids_B_list} not found in tensor_dictionary.")
            return []
    ...
    def move_to(self, destination_path):
        ...
        for file in self.lower_quality:
            try:
                ...
                logging.error(f'Could not move file: {file}')
        logging.info(f'Moved {len(self.lower_quality)} files(s) to "{str(Path(destination_path))}"')
                ...
                if str(usr).lower() == 'y':
                    for file in self.lower_quality:
                        try:
                            ...
                            logging.error(f'Could not delete file: {file}')       
                else:
